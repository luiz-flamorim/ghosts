...
// function generateID(title, location) {
//     return `${title}-${location}`.toLowerCase().replace(/\s+/g, "-").replace(/[^\w-]/g, "").substring(0, 20);
// }
...
async function generateImage(record) {
    ...
    try {
        ...
        console.log(`Generating image ${jsonData.indexOf(record) + 1} of ${jsonData.length}`);
...
async function processImages() {
    for (let record of jsonData) {
        if (!record.img) { // Only generate if there's no local image yet
